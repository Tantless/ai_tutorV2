...
- 正常情况下，先调用 `resolve_assignment.py` 解析唯一合法的今日题目。
...
- `consolidation_practice` 若属于“先发题再作答”的场景，必须先 `record_assignment_delivery.py`，再 `apply_interaction_result.py`。
- 这四类默认不写 `knowledge_update`，也不写 `weak_point_entry`。
